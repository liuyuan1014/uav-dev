package com.uav.dispatch.service.impl;

import cn.hutool.json.JSONUtil;
import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.uav.client.location.LocationFeignClient;
import com.uav.client.mission.MissionFeignClient;
import com.uav.dispatch.config.XxlJobClient;
import com.uav.dispatch.constant.RedisConstant;
import com.uav.dispatch.mapper.MissionJobMapper;
import com.uav.dispatch.service.DispatchService;
import com.uav.dispatch.util.DispatchUtil;
...
public class DispatchServiceImpl implements DispatchService {
     ...
     */
    private static final Integer JOB_GROUP_ID = 2;
    ...
    public Integer startDispatch(StartDispatchForm form) {
        ...
        try {
            // 1. 检查任务是否存在 - 通过Dubbo调用service-mission
            ...
            // 2. 检查是否已经创建过派单任务
            LambdaQueryWrapper<MissionJob> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(MissionJob::getMissionId, missionId);
            MissionJob existingJob = missionJobMapper.selectOne(wrapper);
    ...
    public Boolean stopDispatch(Long missionId) {
        try {
            ...
            LambdaQueryWrapper<MissionJob> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(MissionJob::getMissionId, missionId);
            MissionJob missionJob = missionJobMapper.selectOne(wrapper);
    ...
    public Integer executeDispatch(Long missionId) {
        try {
            ...
            // 2. 检查任务是否已被接单 - 通过Dubbo调用service-mission
            UavMission mission = missionFeignClient.getMissionById(missionId);
            ...
            // 4. 搜索附近空闲飞手 - 通过Dubbo调用service-location
    ...
    public Boolean pushToPilot(Long missionId, Long pilotId) {
        try {
            // 1. 构造任务信息 - 通过Dubbo调用service-mission
            UavMission mission = missionFeignClient.getMissionById(missionId);
            if (mission == null) {
                return false;
            }
    ...
    public Boolean checkAndHandleTimeout(Long missionId) {
        // 通过Dubbo调用service-mission获取任务信息
        UavMission mission = missionFeignClient.getMissionById(missionId);
        if (mission == null) {
            return true;
        }
    ...
    public Boolean acceptMission(Long missionId, Long pilotId) {
        try {
            ...
            // 2. 更新任务状态
            // TODO: 阶段十一将替换为Feign调用service-mission
